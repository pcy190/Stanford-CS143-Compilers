...
public:
  ...
  std::map<Symbol, int> get_class_tag();
...
public:
  ...
  List<CgenNode> *get_children_list() { return children; }
  std::vector<CgenNode *> get_children() {
    std::vector<CgenNode *> ret;
    List<CgenNode> *_children = get_children_list();
  ...
  std::map<Symbol, Symbol> get_dispatch_class_tab();
  std::map<Symbol, Symbol> m_dispatch_class_tab;

  std::map<Symbol, int> get_dispatch_idx_tab();
  int get_dispatch_idx(Symbol name); // m_dispatch_idx_tab[name]
  std::map<Symbol, int> m_dispatch_idx_tab;

  std::vector<attr_class *> get_attr();
  std::vector<attr_class *> m_attribs;

  std::vector<attr_class *> get_full_attr();
  std::vector<attr_class *> m_full_attribs;

  std::map<Symbol, int> get_attr_idx_tab();
...
public:
  ...
  int LookUpAttrib(Symbol sym) {
    std::map<Symbol, int> attrib_idx_tab = m_class_node->get_attr_idx_tab();
