...
public:
  ...
  std::vector<Expression> actual_vec; // parsed actual
public:
  static_dispatch_class(Expression a1, Symbol a2, Symbol a3, Expressions a4) {
    ...
    for (int i = actual->first(); actual->more(i); i = actual->next(i)) {
      actual_vec.push_back(actual->nth(i));
    }
  }
  Expression copy_Expression();
  void dump(ostream &stream, int n);
  std::vector<Expression> get_actual() { return actual_vec; }
...
public:
  ...
  std::vector<branch_class *> cases_list; // vector format
public:
  typcase_class(Expression a1, Cases a2) {
    expr = a1;
    cases = a2;
    for (int i = cases->first(); cases->more(i); i = cases->next(i)) {
      cases_list.push_back((branch_class *)cases->nth(i));
    }
  }
  Expression copy_Expression();
  void dump(ostream &stream, int n);
  std::vector<branch_class *> get_cases() { return cases_list; }
#ifdef Expression_SHARED_EXTRAS
  Expression_SHARED_EXTRAS
#endif
