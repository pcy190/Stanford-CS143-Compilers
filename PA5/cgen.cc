...
int label_idx = 0;
CgenClassTable *codegen_classtable = nullptr; // initialized at cgen
//
...
static void emit_load_address(char *dest_reg, const char *address, ostream &s) {
  emit_load_address(dest_reg, (char *)address, s);
}
...
void CgenClassTable::code_class_nameTab() {
  ...
  for (CgenNode *class_node : class_nodes) {
    ...
    StringEntry *str_entry =
        stringtable.lookup_string(class_name->get_string());

    str << WORD;
    str_entry->code_ref(str);
    str << endl;
    std::vector<CgenNode *> children_vec = class_node->get_children();
    for (CgenNode *_child : children_vec) {
...
void CgenClassTable::code_class_objTab() {
  ...
  std::vector<CgenNode *> class_nodes = this->get_class_nodes();
  for (CgenNode *class_node : class_nodes) {
    Symbol class_name = class_node->name;
    StringEntry *str_entry =
        stringtable.lookup_string(class_name->get_string());
...
std::map<Symbol, int> CgenClassTable::get_class_tag() {
  this->get_class_nodes();
  return m_class_tags;
}
...
std::vector<attr_class *> CgenNode::get_full_attr() {
...
std::vector<attr_class *> CgenNode::get_attr() {
  if (m_attribs.empty()) {
...
std::map<Symbol, int> CgenNode::get_attr_idx_tab() {
  get_full_attr();
...
std::vector<method_class *> CgenNode::get_full_methods() {
  if (m_full_methods.empty()) {
    // update m_full_methods, m_dispatch_class_tab, m_dispatch_idx_tab first
...
std::map<Symbol, Symbol> CgenNode::get_dispatch_class_tab() {
  get_full_methods();
  return m_dispatch_class_tab;
}

std::map<Symbol, int> CgenNode::get_dispatch_idx_tab() {
  get_full_methods();
  return m_dispatch_idx_tab;
}

int CgenNode::get_dispatch_idx(Symbol name) {
  return get_dispatch_idx_tab()[name];
}
...
void method_class::code(ostream &s, CgenNode *class_node) {
  ...
  emit_addiu(SP, SP, get_arg_num() * 4, s);
  s << endl;
...
void CgenNode::code_protObj(ostream &s) {
  std::vector<attr_class *> attribs = get_full_attr();
...
void CgenNode::code_init(ostream &s) {
  ...
  std::vector<attr_class *> attribs = get_attr();
  std::map<Symbol, int> attrib_idx_tab = get_attr_idx_tab();
  for (attr_class *attrib : attribs) {
    s << "\t# init attrib " << attrib->name << endl;
    int idx = attrib_idx_tab[attrib->name];

    if (attrib->init->is_empty()) {
...
CgenClassTable::CgenClassTable(Classes classes, ostream &s)
  ...
  std::map<Symbol, int> class_tags = get_class_tag();
...
void assign_class::code(ostream &s, Environment env) {
  ...
  if ((idx = env.LookUpVar(name)) != -1) {
    ...
    s << "Fatal error! Undefined lvalue type." << endl;
  }
}

void static_dispatch_class::code(ostream &s, Environment env) {
  std::vector<Expression> actuals = this->get_actual();
  ...
  /* _dispatch_abort called when a dispatch is attempted on a void object.
      It Prints the line number, from $t1, and filename, from $a0, at which the
     dispatch
      occurred, and aborts.
   */
  ...
  std::string tab_addr = type_name->get_string();
  tab_addr += DISPTAB_SUFFIX;
  s << "\t# caller to dispatcher field" << endl;
  emit_load_address(T1, tab_addr.c_str(), s);
  emit_load(T1, _class_node->get_dispatch_idx(name), T1, s);
  emit_jalr(T1, s);
  s << endl;
}
...
void dispatch_class::code(ostream &s, Environment env) {
  ...
  s << "\t# jump to dispatch_tab[offset]" << endl;
  emit_load(T1, 2, ACC, s);
  s << endl;

  int idx = _class_node->get_dispatch_idx(name);
...
void cond_class::code(ostream &s, Environment env) {
  ...
  int label_false = label_idx++;
  int label_finish = label_idx++;
...
void typcase_class::code(ostream &s, Environment env) {
  std::map<Symbol, int> _class_tags = codegen_classtable->get_class_tag();
  ...
  std::vector<branch_class *> _cases = get_cases();
  int labelbeg = label_idx;
  int finish = label_idx + _cases.size();
  int caseidx = 0;
  label_idx += _cases.size() + 1;
  ...
  auto GetChildrenTagsSet = [&](std::vector<int> __curr_tags) {
    ...
    for (int __curr_tag : __curr_tags) { // find children of this class.
      ...
      std::vector<CgenNode *> __children_nodes = __curr_node->get_children();
      for (CgenNode *__children_node : __children_nodes) {
        int __children_tag = _class_tags[__children_node->name];
        if (std::find(__children_tags.begin(), __children_tags.end(),
                      __children_tag) == __children_tags.end()) {
...
void let_class::code(ostream &s, Environment env) {
  ...
  if (init->is_empty()) {
    // basic types with default init value
    if (type_decl == Str) {
      emit_load_string(ACC, stringtable.lookup_string(""), s);
